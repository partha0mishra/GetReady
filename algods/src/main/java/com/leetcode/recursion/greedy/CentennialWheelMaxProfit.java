package com.leetcode.recursion.greedy;
/*
 ...
 */
public class CentennialWheelMaxProfit {
	public int minOperationsMaxProfit(int[] customers, int boardingCost, int runningCost) {
		if(runningCost > boardingCost*4) return -1;
		int maxProfit=0, rotated=0;
		int leftOver=0, rotation=0,profit=0,totalBoarded=0;
		for(int i=0;;i++) {
			int boarded=0, available=leftOver;
			...
			rotation+=1;
			profit=totalBoarded*boardingCost - rotation*runningCost;
//			System.out.println("profit: "+profit);
			if(profit > maxProfit) {
				maxProfit=profit;
				rotated=rotation;
			}
			if(i >= customers.length && leftOver==0) break;
		}
		
		return (maxProfit < 0)? -1:rotated;
    }
	public static void main(String[] args) {
		CentennialWheelMaxProfit instance= new CentennialWheelMaxProfit();
		System.out.println(instance.minOperationsMaxProfit(new int[] {8,3}, 5, 6));//37
		System.out.println(instance.minOperationsMaxProfit(new int[] {10,9,6}, 6,4));//7
		System.out.println(instance.minOperationsMaxProfit(new int[] {3,4,0,5,1}, 1,92));//-1
		System.out.println(instance.minOperationsMaxProfit(new int[] {10,10,6,4,7}, 3,8));//9
	}
