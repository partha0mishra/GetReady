...
import com.algods.sedgewick.sort.elementary.InsertionSort;
...
//		if (nums[mid+1] >= nums[mid]) {// All the numbers are already in order
...
//        }
	...
	private void merge(int[] nums, int[] aux, int lo, int mid, int hi) {
		...
		for(int k=lo; k<=hi; k++) {
			...
			else if (aux[j] < aux[i])	nums[k]=aux[j++];// to ensure stability, don't use ==
			else						nums[k]=aux[i++];
		}
	}
...
//		printArray(nums);
		...
		MergeSortWithInsertion instance= new MergeSortWithInsertion();
